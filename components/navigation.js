...
export default function Navigation() {
  ...
  return (
    <>
      <nav className="fixed top-0 w-full bg-white/85 backdrop-blur-md border-b border-slate-200/80 z-50 transition-all duration-300">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative text-sm font-medium transition-colors hover:text-orange-600 after:absolute after:left-0 after:-bottom-1.5 after:h-0.5 after:bg-orange-600 after:transition-all ${
                    pathname === link.href ? "text-orange-600 after:w-full" : "text-slate-700 after:w-0 hover:after:w-full"
                  }`}
                >
                  {link.label}
                </Link>
              ...
              <Link href='mailto:info@baroquevariations.com'>
                <Button className="bg-orange-600 hover:bg-orange-700 text-white rounded-full px-6">Email Us</Button>
              </Link>
            </div>
